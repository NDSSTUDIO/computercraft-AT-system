...
local function hud(speed,gear,rpm,relspeed)
    ...
    monitor.write("-----------------------------------------------------------")
    monitor.setCursorPos((hud_scale / 2 - 5) - rpmstr*5,(hud_scale / 2) + 2)
    monitor.write("+----+----+----+----+----+----+----+----+----+----+----+---------")
    monitor.setCursorPos((hud_scale / 2 - 5) - rpmstr*5,(hud_scale / 2) + 3)
...
while true do
    local joy_y = joy.getAxis(2) -- 获取手柄参数
    local truespeed = ship.getVelocity() -- 获取速度分量
    local relspeed = math.sqrt(truespeed.x^2 + truespeed.z^2) -- 计算真实速度(m/s)
    ...
    if throttle > 0 then -- 加速
        ...
        engine_speed = math.max(engine_speed - deceleration, 0)  
        -- 引擎速度-减速率 最小0rpm
    end
    ...
    if joy_y < 0 then -- 油门：FULL
        if gear == -1 then
                ...
                throttle = 1  -- 油门开度:100%
            if relspeed < 3 then -- AT1档
                load = 1.2 * mass -- 负载
                gear = 1
            elseif relspeed < 13 and relspeed >= 5 then -- AT2档  *2
                load = 2.0 * mass -- 负载
                gear = 2
            elseif relspeed < 24 and relspeed >= 13 then -- AT3档 *3
                load = 2.5 * mass -- 负载
                gear = 3
            elseif relspeed < 38 and relspeed >= 24 then -- AT4档 *4
                load = 3 * mass -- 负载
                gear = 4
            elseif relspeed >= 38 then -- AT5档 *5
                load = 4.5 * mass -- 负载
                gear = 5
            end
        end
